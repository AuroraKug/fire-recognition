...
class FireCNN_v4(nn.Module):
    '''
    Description
        FireCNN_v4 在v3骨干上引入多层级全局特征融合 (stage2+stage3+stage4)
    Args
...
def build_model(num_classes: int, pretrained: bool = False) -> nn.Module:
    '''
    Description
        构建 FireCNN_v4 模型 忽略 pretrained 参数确保随机初始化
    Args
        num_classes (int): 分类类别数
        pretrained (bool): 预训练标志 将被忽略
    Returns
        model (nn.Module): FireCNN_v4 实例
    '''
    if pretrained:
        print("Warning: FireCNN_v4 does not load pretrained weights; using random initialization.")
    model = FireCNN_v4(num_classes=num_classes)
    return model
